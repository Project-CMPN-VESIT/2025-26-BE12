...
def _quality_reason(shot, quality, signals: dict) -> str:
    """
    Builds a plain-text reason string driven entirely by the actual
    normalised column values — not hardcoded text.

    Rule:
      value < 0.40  → clearly bad   (named as a fault)
      0.40–0.60     → borderline    (named as 'weak' or 'partial')
      value > 0.60  → good          (named as a strength for GOOD shots)

    'signals' is a dict of {label: normalised_value} for every metric
    that is relevant to this shot type.  0/1 flags are treated as 0.0/1.0.
    """
    faults    = []   # below 0.40
    borderline= []   # 0.40–0.60
    strengths = []   # above 0.60

    for label, val in signals.items():
        v = float(val)
        if v < 0.40:
            faults.append(f"{label}={v:.2f}")
        elif v <= 0.60:
            borderline.append(f"{label}={v:.2f}")
        else:
            strengths.append(f"{label}={v:.2f}")

    if quality == "GOOD":
        parts = [f"strengths: {', '.join(strengths)}"] if strengths else []
        if borderline:
            parts.append(f"borderline: {', '.join(borderline)}")
        return " | ".join(parts) if parts else "all signals strong"

    if quality == "AVERAGE":
        parts = []
        if borderline:
            parts.append(f"weak: {', '.join(borderline)}")
        if faults:
            parts.append(f"poor: {', '.join(faults)}")
        if strengths:
            parts.append(f"ok: {', '.join(strengths)}")
    ...
    if borderline:
        parts.append(f"weak: {', '.join(borderline)}")
    return " | ".join(parts) if parts else "all signals weak"


# Per-shot: which signals matter for grading
SHOT_SIGNALS = {
    "SMASH":   ["wrist_above_nose", "wrist_above_shoulder", "jump_at_contact",
                "elbow_straight",   "shuttle_near_wrist",   "contact_height_norm",
                "elbow_extension_norm", "wrist_speed_norm"],

    "CLEAR":   ["wrist_above_nose", "wrist_above_shoulder", "elbow_straight",
                "elbow_extension_norm", "shuttle_near_wrist", "wrist_speed_norm",
                "contact_height_norm"],

    "DRIVE":   ["wrist_above_waist", "wrist_above_shoulder", "shuttle_near_wrist",
                "shuttle_speed_norm", "wrist_speed_norm", "elbow_extension_norm",
                "body_symmetry_norm"],

    "DROP":    ["wrist_above_shoulder", "shuttle_near_wrist", "contact_height_norm",
                "wrist_speed_norm", "elbow_extension_norm"],

    "NETSHOT": ["shuttle_near_wrist", "wrist_speed_norm", "stance_width_norm",
                "body_symmetry_norm", "shuttle_dist_norm"],

    "LIFT":    ["wrist_above_waist", "shuttle_near_wrist", "wrist_speed_norm",
                "stance_width_norm", "jump_height_norm", "elbow_extension_norm"],
}
...
class CSVLogger:
    ...
    BIO_COLS = [
        ...
        # ── Dynamics ────────────────────────────────────────────────────────
        "jump_flag",            # 0 or 1
        "jump_height_norm",     # estimated ankle rise / body_ref*0.6
    ...
    SHOT_COLS = BIO_COLS + [
        ...
        "shuttle_direction",    # UP / DOWN / FLAT / STATIC at contact moment
        "shuttle_near_wrist",   # 1 if shuttle within 200px of wrist at contact
        # ── Timing ──────────────────────────────────────────────────────────
        "reaction_time_s",      # seconds since player's last shot
        "reaction_time_norm",   # / MAX_REACTION_S (0=instant, 1=very slow)
        # ── Contact quality signals (the actual grading inputs) ──────────────
        "contact_height_norm",  # 1-(wrist_y/frame_h): 1=top of frame, 0=bottom
        "wrist_above_nose",     # 1 if wrist higher than nose at contact (good overhead)
        "wrist_above_shoulder", # 1 if wrist above shoulder level
        "wrist_above_waist",    # 1 if wrist above waist level
        "jump_at_contact",      # 1 if player was jumping at contact moment
        "elbow_straight",       # 1 if elbow_extension_norm > 0.80 (arm extended)
        # ── Recovery ────────────────────────────────────────────────────────
        "recovery_score",       # 0-1: how centred player is after shot
        # ── Grade breakdown (human-readable why) ────────────────────────────
    ]
    ...
    def log_biomechanics(self, frame_idx, player, shuttle,
        ...
        # Jump — temporal method first, then keypoint fallbacks
        waist_y = (float(a(11)[1]) + float(a(12)[1])) / 2
        ankles  = [float(a(i)[1]) for i in [15, 16] if a(i)[0] > 5 and a(i)[1] > 5]
        knees   = [float(a(i)[1]) for i in [13, 14] if a(i)[0] > 5 and a(i)[1] > 5]
        jump_flag, jump_h_n = 0, 0.0

        # Method 1: temporal feet-y baseline (most reliable)
        if feet_hist is not None and len(feet_hist) >= 10 and body_ref > 0:
            baseline = float(np.median(list(feet_hist)[-10:]))
            current  = float(feet_hist[-1])
            if baseline - current > body_ref * 0.15:
                jump_flag = 1
                rise      = baseline - current
                jump_h_n  = norm_dist(rise, body_ref * 0.8)

        # Method 2: ankle keypoints
        if not jump_flag and ankles and body_ref > 0:
            threshold = waist_y + body_ref * 0.75
            if min(ankles) < threshold:
                jump_flag = 1
                rise      = threshold - min(ankles)
                jump_h_n  = norm_dist(rise, body_ref * 0.8)

        # Method 3: knee fallback
        if not jump_flag and knees and body_ref > 0:
            if min(knees) < waist_y + body_ref * 0.25:
                jump_flag = 1
                rise      = (waist_y + body_ref * 0.25) - min(knees)
                jump_h_n  = norm_dist(rise, body_ref * 0.3)
        ...
        row = {
            ...
            "jump_flag":            jump_flag,
            "jump_height_norm":     round(jump_h_n, 4),
    ...
    # ── 3. Shot events ────────────────────────────────────────────────────────
    MAX_SHUTTLE_KMH = 400.0   # realistic max shuttle speed for normalisation

    def log_shot(self, bio_row, shot, quality, tip,
                 peak_speed_px, shuttle_direction, fps, px_per_meter,
                 frame_h, player_kpts, shuttle_tracker=None):
        """
        peak_speed_px : peak shuttle speed px/frame captured during swing
        shuttle_tracker: passed so we can use rolling avg as fallback
        """
        pid = bio_row["player_id"]

        # ── Shuttle speed with 3-level fallback ───────────────────────────
        if px_per_meter > 0 and fps > 0:
            spd_kmh = peak_speed_px * fps * 3.6 / px_per_meter
        else:
            spd_kmh = 0.0

        # Level 2: rolling avg of recent non-zero speeds from this session
        if spd_kmh < 1.0 and shuttle_tracker is not None:
            avg_px = shuttle_tracker.recent_avg_speed()
            if avg_px > 0 and px_per_meter > 0 and fps > 0:
                spd_kmh = avg_px * fps * 3.6 / px_per_meter

        # Level 3: shot-type fallback (fixed realistic value 50-100 km/h)
        # Used when shuttle model couldn't detect shuttle at all during rally
        if spd_kmh < 1.0:
            spd_kmh = float(ShuttleTracker.SHOT_SPEED_FALLBACK.get(shot, 70))

        spd_norm = norm_dist(spd_kmh, self.MAX_SHUTTLE_KMH)
        ...
        k = player_kpts
        def kp(i): return k[i] if len(k) > i and k[i][0] > 1 else np.array([0.0,0.0])
        nose_y  = float(kp(0)[1])
        avg_sh  = (float(kp(5)[1]) + float(kp(6)[1])) / 2
        avg_wa  = (float(kp(11)[1]) + float(kp(12)[1])) / 2
        wrist   = self._dominant_wrist(k)
        wrist_y = float(wrist[1]) if wrist is not None else avg_wa

        wrist_above_nose     = int(wrist_y < nose_y)
        wrist_above_shoulder = int(wrist_y < avg_sh)
        wrist_above_waist    = int(wrist_y < avg_wa)
        jump_at_contact      = int(bio_row["jump_flag"])
        elbow_straight       = int(bio_row["elbow_extension_norm"] > 0.80)
        shuttle_near         = int(bio_row["shuttle_dist_norm"] < 0.15)

        # ── Build signals dict from ACTUAL column values for this shot ────
        # Uses SHOT_SIGNALS to know which columns are relevant per shot type.
        # Values are taken directly from bio_row (already normalised 0-1)
        # plus the extra flags computed above (0 or 1 = 0.0 or 1.0).
        all_values = {
            **bio_row,
            "wrist_above_nose":     float(wrist_above_nose),
            "wrist_above_shoulder": float(wrist_above_shoulder),
            "wrist_above_waist":    float(wrist_above_waist),
            "jump_at_contact":      float(jump_at_contact),
            "elbow_straight":       float(elbow_straight),
            "shuttle_near_wrist":   float(shuttle_near),
            "shuttle_speed_norm":   spd_norm,
            "contact_height_norm":  contact_h_n,
        }

        relevant_cols = SHOT_SIGNALS.get(shot, [])
        signals = {
            col: round(float(all_values.get(col, 0.0)), 3)
            for col in relevant_cols
            if col in all_values
        }

        reason = _quality_reason(shot, quality, signals)
        ...
        row = {
            **bio_row,
            ...
            "shuttle_near_wrist":   shuttle_near,
            ...
            "wrist_above_waist":    wrist_above_waist,
            "jump_at_contact":      jump_at_contact,
...
class ShuttleTracker:
    ...
    # Fallback speeds (km/h) per shot type when shuttle model misses contact
    # Values are realistic for club/amateur play — within 50-100 range as requested
    SHOT_SPEED_FALLBACK = {
        "SMASH":   95,
        "DRIVE":   80,
        "CLEAR":   65,
    ...
    def __init__(self, model_path, history_len=25):
        ...
        self._alpha         = 0.4
        self._frames_lost   = 0
        # Rolling buffer of recent NON-ZERO speeds (last 60 frames)
        # Used as fallback when shuttle is lost at contact moment
        self._speed_buffer  = deque(maxlen=60)
    ...
    def update(self, results):
        if results and len(results.boxes)>0:
            ...
            if len(self.smooth_history)>=3:
                ...
                # Store non-zero speeds in rolling buffer for fallback
                if self.speed > 2.0:
                    self._speed_buffer.append(self.speed)
    ...
    def recent_avg_speed(self) -> float:
        """
        Returns average of recent non-zero speed readings.
        Used as fallback when shuttle is lost at contact moment (motion blur).
        Returns 0 if no recent speed data.
        """
        if not self._speed_buffer:
            return 0.0
        return float(np.mean(list(self._speed_buffer)[-20:]))
...
class ShotAnalyser:
    ...
    def __init__(self, transformer):
        ...
        # Temporal jump detection: track feet-y over last 20 frames per player
        # When player jumps, their whole body rises → feet-y decreases
        self.feet_hist:dict={}   # pid → deque of feet_y values

    def analyse(self, player, shuttle, rackets: dict = None):
        ...
        # ── Temporal jump tracking: record feet-y every frame ────────────
        if pid not in self.feet_hist:
            self.feet_hist[pid] = deque(maxlen=20)
        self.feet_hist[pid].append(float(player['feet'][1]))

        if len(self.wrist_hist.get(pid,[]))<5: return "","","",0.0
        if time.time()-self.last_shot_ts.get(pid,0)<self.SHOT_COOLDOWN: return "","","",0.0

        zone   = self._get_zone(player['feet'])
        racket = (rackets or {}).get(pid)

        if racket and not racket.get('estimated', False):
            contact_pt = racket['head']
        else:
            contact_pt = tuple(wrist) if wrist is not None else player['feet']
        near_shuttle = shuttle.distance_to(contact_pt) < 200

        current_spd  = shuttle.speed
        raw_shot, raw_quality = self._detect(pid,kpts,shuttle,zone,near_shuttle,racket)
        ...
        if raw_shot:
            ...
            self.shot_buffer[pid].append((raw_shot,raw_quality,current_spd))
        else:
            ...
            if buffered:
                ...
                if self.gap_counter[pid]<self.MAX_GAP.get(cur,0):
                    ...
                    last=self.shot_buffer[pid][-1]
                    self.shot_buffer[pid].append((cur,last[1],current_spd))
        ...
        if self.shot_buffer[pid]:
            ...
            if len(self.shot_buffer[pid])>=needed:
                ...
                peak_spd=max(s[2] for s in self.shot_buffer[pid])
                self.shot_buffer[pid]=[]; self.gap_counter[pid]=0
                self.last_shot_ts[pid]=time.time()
                return dominant,quality,tip,peak_spd
        return "","","",0.0
    ...
    def _is_jumping(self, k, p_ref, pid=None):
        """
        Multi-method jump detection:

        Method 1 (temporal — most reliable):
          Track feet-y over last 20 frames. A jump is detected when current
          feet-y is significantly lower than the recent baseline.
          Works even when ankle keypoints are 0 (whole body rises = feet rise).

        Method 2 (ankle keypoints):
          If ankles detected, check they're unusually high vs waist.

        Method 3 (knee fallback):
          If only knees detected, check they're above waist level.
        """
        waist_y = (k[11][1] + k[12][1]) / 2

        # ── Method 1: temporal feet-y baseline ────────────────────────────
        if pid is not None and pid in self.feet_hist:
            hist = self.feet_hist[pid]
            if len(hist) >= 10:
                # Baseline = median of last 10 readings (robust to outliers)
                baseline = float(np.median(list(hist)[-10:]))
                current  = float(hist[-1])
                # Jump = feet rose more than 15% of body_ref above baseline
                if baseline - current > p_ref * 0.15:
                    return True

        # ── Method 2: ankle keypoints ─────────────────────────────────────
        # Check BOTH x and y > 5 to confirm keypoint is actually detected
        ankles = [a[1] for a in [k[15], k[16]] if a[0] > 5 and a[1] > 5]
        if ankles and waist_y > 0:
            # Standing: ankle_y ≈ waist_y + 1.2*p_ref
            # Threshold 0.75: ankle must have risen at least 0.45*p_ref
            return min(ankles) < (waist_y + p_ref * 0.75)

        # ── Method 3: knee keypoints ─────────────────────────────────────
        knees = [kn[1] for kn in [k[13], k[14]] if kn[0] > 5 and kn[1] > 5]
        if knees and waist_y > 0:
            return min(knees) < (waist_y + p_ref * 0.25)
    ...
    def _detect(self, pid, k, shuttle, zone, near_shuttle, racket=None):
        if len(k)<17: return "",""
        ...
        jumping=self._is_jumping(k,p_ref,pid); dx,dy=self._swing_motion(pid)
        s_dir=shuttle.direction; s_spd=shuttle.speed; elbow_a=self._elbow_angle(k)

        # Use actual racket head when available — more accurate than wrist
        # Falls back to wrist if racket not detected or is estimated (low conf)
        if racket and not racket.get('estimated', False):
            contact_y    = float(racket['head'][1])
            racket_angle = float(racket['angle'])   # 0=horizontal, 90=vertical
        else:
            contact_y    = float(wrist[1])
            racket_angle = None

        if (contact_y<=nose_y+p_ref*0.15 or jumping) and dy>p_ref*0.16:
            shot="SMASH"
            if jumping and contact_y<nose_y and elbow_a>155: quality="GOOD"
            elif contact_y<=nose_y and elbow_a>130:           quality="AVERAGE"
            else:                                              quality="BAD"
            if near_shuttle and s_dir=="DOWN" and s_spd>12:
                quality="GOOD" if quality=="AVERAGE" else quality
            return shot,quality

        if (contact_y<avg_sh and dy>p_ref*0.12
                and zone in ("MID","BACK","UNKNOWN") and s_dir in ("UP","STATIC")):
            shot="CLEAR"
            if contact_y<nose_y and elbow_a>145 and dx>p_ref*0.08: quality="GOOD"
            elif contact_y<avg_sh: quality="AVERAGE"
            else:                  quality="BAD"
            return shot,quality

        if (contact_y<avg_sh and 0<dy<p_ref*0.14
        ...
        if (avg_sh-p_ref*0.30<contact_y<avg_wa+p_ref*0.50
                and dx>p_ref*0.22 and abs(dy)<p_ref*0.12):
            shot="DRIVE"
            # Racket angle bonus: nearly horizontal (< 30°) = flat drive = GOOD
            angle_good = (racket_angle is not None and racket_angle < 30)
            if (near_shuttle and s_dir=="FLAT" and s_spd>10) or angle_good:
                quality="GOOD"
        ...
        if (at_net and abs(dx)<p_ref*0.28 and abs(dy)<p_ref*0.22
                and contact_y>avg_sh-p_ref*0.4
                and contact_y<avg_wa+p_ref*0.5 and not jumping):
            shot="NETSHOT"
        ...
        if contact_y>avg_wa and dy<-(p_ref*0.14):
            shot="LIFT"
...
class PlayerIDManager:
    ...
    MAX_PLAYERS      = 2
    LOST_TIMEOUT_S   = 5.0
    REASSIGN_DIST_PX = 150

    # P1 = far side (top of frame), P2 = near side (bottom of frame)
    ID_COLORS = {
        1: (255, 100, 0),    # blue  — far side player
        2: (0,   200, 255),  # yellow — near side player
    }
    ...
    def _position_based_id(self, feet) -> int:
        """
        P1 = player with lower y in image (far side / top of frame)
        P2 = player with higher y in image (near side / bottom of frame)
        """
        if not self._slots:
            return 1   # first detected player always P1

        # Compare new player's y to existing player's y
        existing = list(self._slots.items())  # [(sid, slot), ...]
        used_ids = set(self._slots.keys())

        if len(existing) == 1:
            existing_sid, existing_slot = existing[0]
            existing_y = existing_slot['last_pos'][1]
            new_y      = feet[1]

            if new_y < existing_y:
                # New player is higher up (smaller y) — should be P1
                # Swap existing to P2 if needed
                if existing_sid == 1:
                    # Remap existing P1 → P2, assign P1 to new
                    self._slots[2] = self._slots.pop(1)
                    tid = self._slots[2]['tracker_id']
                    self._tracker_to_stable[tid] = 2
                    return 1
                return 1 if 1 not in used_ids else 2
            else:
                return 2 if 2 not in used_ids else 1

        # Both slots taken — fallback (shouldn't reach here due to MAX_PLAYERS cap)
        return next((i for i in [1, 2] if i not in used_ids), 1)
...
# ─────────────────────────────────────────────────────────────────────────────
# 6b. RACKET DETECTOR  (Option 2: COCO tennis racket fallback)
# ─────────────────────────────────────────────────────────────────────────────
class RacketDetector:
    """
    Uses YOLOv8n pretrained on COCO, class 38 = tennis racket.
    Works out of the box — no training required.
    Accuracy ~50% on badminton rackets (better than pure wrist estimation).

    Runs every INFER_EVERY_N frames to save compute.
    Results are cached between inferences so every frame still gets data.

    When a trained racket_best.pt is available later, just set:
        LOCAL_MODEL_PATH = "models/racket_best.pt"
    and it switches automatically.
    """
    COCO_RACKET_CLASS = 38      # tennis racket in COCO
    INFER_EVERY_N     = 3       # run inference every 3 frames
    CONTACT_DIST_PX   = 55      # shuttle within this of racket head = contact
    ASSIGN_DIST_PX    = 350     # max dist to assign racket to a player

    def __init__(self, local_model_path="models/racket_best.pt"):
        self._frame_cnt  = 0
        self._last_res   = None
        self.head_history: dict = {}   # pid → deque of (x,y)

        # Use local trained model if it exists, else fall back to COCO
        if os.path.exists(local_model_path):
            self._model      = YOLO(local_model_path)
            self._use_coco   = False
            print(f"✅ RacketDetector: trained model ({local_model_path})")
        else:
            self._model      = YOLO("yolov8n.pt")
            self._use_coco   = True

    def predict(self, frame):
        """Runs every INFER_EVERY_N frames, returns cached result otherwise."""
        self._frame_cnt += 1
        if self._frame_cnt % self.INFER_EVERY_N != 0:
            return self._last_res
        if self._use_coco:
            res = self._model(frame, verbose=False,
                              conf=0.22, classes=[self.COCO_RACKET_CLASS])[0]
        else:
            res = self._model(frame, verbose=False, conf=0.35)[0]
        self._last_res = res
        return res

    def get_rackets(self, result, players: list) -> dict:
        """
        Returns {player_id: {'head':(x,y), 'angle':deg, 'box':(...), 'conf':f}}
        """
        ...
        for box in result.boxes:
            ...
            # Racket orientation from bounding box shape
            angle_deg = math.degrees(math.atan2(bh, bw)) if bw > 0 else 90.0

            # Assign to nearest player within threshold
            best_pid, best_dist = None, float(self.ASSIGN_DIST_PX)
            for p in players:
                dist = math.hypot(hx - p['feet'][0], hy - p['feet'][1])
                if dist < best_dist:
                    best_dist = dist
                    best_pid  = p['id']

            if best_pid is not None:
                if best_pid not in self.head_history:
                    self.head_history[best_pid] = deque(maxlen=10)
                self.head_history[best_pid].append((hx, hy))
                assigned[best_pid] = {
                    'head':  (hx, hy),
                    'angle': angle_deg,
                    'box':   (x1, y1, x2, y2),
    ...
    def is_contact(self, pid: int, shuttle_pos) -> bool:
        h = self.head_history.get(pid)
        if not h or shuttle_pos is None: return False
        return math.hypot(h[-1][0]-shuttle_pos[0],
                          h[-1][1]-shuttle_pos[1]) < self.CONTACT_DIST_PX

    def draw(self, frame, rackets: dict):
        for pid, r in rackets.items():
            x1,y1,x2,y2 = r['box']
            # Colour: green if real detection, grey if estimated
            col = (0,255,180) if not r.get('estimated') else (140,140,140)
            cv2.rectangle(frame,(x1,y1),(x2,y2),col,2)
            hx,hy = r['head']
            cv2.circle(frame,(hx,hy),5,col,-1)
            label = f"R·P{pid} {r['conf']:.0%}"
            if r.get('estimated'): label = f"R·P{pid} est"
            cv2.putText(frame,label,(x1,y1-6),
                        cv2.FONT_HERSHEY_SIMPLEX,0.38,col,1)
        return frame


class WristFallbackEstimator:
    """
    When COCO misses the racket for a player, estimates racket head by
    projecting the dominant arm vector (elbow→wrist) forward by racket length.
    Marked with 'estimated': True and conf=0.20 so quality grading weights
    it appropriately lower than a real detection.
    """
    RACKET_LEN_RATIO = 0.55

    def fill_gaps(self, players: list, rackets: dict) -> dict:
        result = dict(rackets)
        for p in players:
            if p['id'] in result: continue
            k   = p['kpts']
            est = self._project(k)
            if est:
                result[p['id']] = est
    ...
    def _project(self, k):
        if len(k) < 17: return None
        def kp(i): return k[i] if len(k)>i and k[i][0]>1 else None
        wr,wl = kp(10),kp(9)
        er,el = kp(8), kp(7)
        sr,sl = kp(6), kp(5)
        if wr is not None and wl is not None:
            arm = (sr,er,wr) if wr[1]<wl[1] else (sl,el,wl)
        elif wr is not None: arm=(sr,er,wr)
        elif wl is not None: arm=(sl,el,wl)
        else: return None

        shoulder,elbow,wrist = arm
        if any(x is None for x in [shoulder,elbow,wrist]): return None
        arm_vec = np.array(wrist)-np.array(elbow)
        arm_len = np.linalg.norm(arm_vec)
        if arm_len<10: return None
        sw_len = np.linalg.norm(np.array(wrist)-np.array(shoulder))
        if sw_len<20: return None

        racket_len = sw_len*self.RACKET_LEN_RATIO
        arm_unit   = arm_vec/arm_len
        tip        = np.array(wrist)+arm_unit*racket_len
        hx         = int((wrist[0]+tip[0])/2)
        hy         = int((wrist[1]+tip[1])/2)
        angle      = math.degrees(math.atan2(arm_unit[1],arm_unit[0]))
        bw,bh      = int(racket_len*0.35), int(racket_len*1.1)
        return {
            'head':      (hx, hy),
            'angle':     angle,
            'box':       (hx-bw//2, hy-bh//2, hx+bw//2, hy+bh//2),
            'conf':      0.20,
            'estimated': True,
        }
...
def main():
    ...
    racket_detector  = RacketDetector()   
    wrist_fallback   = WristFallbackEstimator()
    ...
    cap          = cv2.VideoCapture("data/raw/badminton.mp4")
    ...
    while True:
        ...
        # Racket detection + wrist fallback for missed frames
        r_res   = racket_detector.predict(frame)
        rackets = racket_detector.get_rackets(r_res, players)
        rackets = wrist_fallback.fill_gaps(players, rackets)

        for p in players:
            shot,quality,tip,peak_spd=shot_analyser.analyse(
                p, shuttle_tracker, rackets)
            ...
            bio_row = csv_logger.log_biomechanics(
                ...
                feet_hist=shot_analyser.feet_hist.get(p['id']))
            ...
            if shot:
                ...
                csv_logger.log_shot(
                    ...
                    peak_speed_px=peak_spd,
                    shuttle_direction=shuttle_tracker.direction,
                    fps=fps, px_per_meter=px_per_meter,
                    frame_h=frame_h, player_kpts=p['kpts'],
        ...
        disp=racket_detector.draw(disp,rackets)
